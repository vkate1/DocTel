    ...
    return (
        <div className='eventbox'>
            {treatEv?.event === "PrescriptionAddedTreat" || treatEv?.event === "ReportAddedTreat" ?
                <a href={treatEv?.returnValues.report || treatEv?.returnValues.prescription} target="_blank"><img src={treatEv?.returnValues.report || treatEv?.returnValues.prescription}/></a> : null
            }
            <h6>Event: {treatEv?.event}</h6>
            <p>{treatEv?.event === "doctorAddedTreat" ? <p>Doctor: {treatEv?.returnValues.docAadhar}</p> : null }</p>
            <p>{treatEv?.event === "PrescriptionAddedTreat" ? <p style={{"word-wrap":"break-word"}}>Prescription: {treatEv?.returnValues.prescription}</p> : null }</p>
            <p>{treatEv?.event === "ReportAddedTreat" ? <p style={{"word-wrap":"break-word"}}>Report: {treatEv?.returnValues.report}</p> : null }</p>
...
class TreatmentHistoryComp extends Component {
    ...
    async componentDidMount(){
        var rex = await this.props.contract?.methods.treatments(this.props.matchId).call();          
        ...
        return (
            <div className='body_style'>
                ...
                <div className='details'>
                    <p><span className='column1'>Treatment ID</span> <span className='column2'>:  {this.state.treatment?.treatment_Id}</span></p>
                    <p><span className='column1'>Patient Aadhar</span> <span className='column2'>:  {this.state.treatment?.patientAadhar}</span></p>
                    <p><span className='column1'>Admin Aadhar</span> <span className='column2'>:  {this.state.treatment?.adminAadhar}</span></p>
                </div>
                <hr/>
                <h2>Events</h2>
                <br />
